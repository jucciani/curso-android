...
public class ImageDownloadRunnable implements Runnable {
    ...
    private static final String LOG_TAG = "ImageDownloadRunnable";
     ...
     * ImageTask passes itself to an ImageDownloadRunnable instance through the
     * ImageDownloadRunnable constructor, after which the two instances can access each other's
     * variables.
     */
    public interface TaskRunnableDownloadMethods {
        ...
        /**
         * Defines the actions for each state of the ImageTask instance.
     ...
     *
     * @param imageTask The ImageTask, which implements TaskRunnableDecodeMethods
     */
    public ImageDownloadRunnable(TaskRunnableDownloadMethods imageTask) {
    ...
    public void run() {
        ...
        try {
            ...
            if (null == byteBuffer) {
                ...
                /*
                 * Calls the ImageTask implementation of {@link #handleDownloadState} to
                 * set the state of the download
                 */
                mImageTask.handleDownloadState(HTTP_STATE_STARTED);
                ...
                try {
                    Log.d("Fetching image: ", mImageTask.getImageURL().toString());

                    // Opens an HTTP connection to the image's URL
                    HttpURLConnection httpConn =
                            (HttpURLConnection) mImageTask.getImageURL().openConnection();

                    // Sets the user agent to report to the server
                    //httpConn.setRequestProperty("User-Agent", Constant.USER_AGENT);
            ...
            /*
             * Stores the downloaded bytes in the byte buffer in the ImageTask instance.
             */
            mImageTask.setByteBuffer(byteBuffer);

            /*
             * Sets the status message in the ImageTask instance. This sets the
            ...
            if (null == byteBuffer) {
                mImageTask.handleDownloadState(HTTP_STATE_FAILED);
            }

            /*
             * The implementation of setHTTPDownloadThread() in ImageTask calls
             * ImageTask.setCurrentThread(), which then locks on the static ThreadPool
